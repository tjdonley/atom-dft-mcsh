...
from .hf import CoulombCouplingCalculator, HartreeFockExchange
...
class OEPCalculator(HartreeFockExchange, RPACorrelation):
    ...
    def compute_oep_potentials(
        ...
        # Zero point shift correction is not needed because `self.ops_builder_oep.physical_nodes`
        # already correctly handles the boundary conditions. The boundary condition application ensures
        # that the OEP potential has the correct asymptotic behavior, making the zero point shift
        ...
        oep_coefficient = self._apply_minus_one_over_r_boundary_condition(
            ...
            r_nodes     = self.ops_builder_oep.physical_nodes,
            r_cutoff    = 9.0,
        )
    ...
    def compute_oep_energy_densities(
        ...
        if not self.use_rpa_correlation:
            ...
            e_c_oep = self.compute_correlation_energy_density(
                full_eigen_energies    = full_eigen_energies,
                full_orbitals          = full_orbitals,
                full_l_terms           = full_l_terms,
                enable_parallelization = enable_parallelization,
    ...
    def _compute_rpa_correlation_driving_term(
        ...
        l_couple_max = (l_occ_max + l_unocc_max).astype(np.int32)
